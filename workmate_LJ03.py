# -*- coding: utf-8 -*-
"""
Created on Fri Dec 18 12:09:30 2020

Agent with random layer L and S
...
class WorkMATe(object):
    ...
    def __init__(self, env=None, nhidden=15, nblocks=2, block_size=15):
        ...
        #self.W_lx = np.random.randn(nl, nx) * np.sqrt(2./nx) #He initialisation
        ...
        #self.W_hl = np.random.randn(nh, nl + nblocks + 1) * np.sqrt(2./(nl + nblocks + 1))
        # connections S->h
        self.W_hS  = np.random.sample( (nh, nS    ) ) * (wh-wl) + wl
        #self.W_hS = np.random.randn(nh, nS) * np.sqrt(2./nS)
        # connections h->q:
        self.W_qh  = np.random.sample( (nq, nh + 1) ) * (wh-wl) + wl
        #self.W_qh = np.random.randn(nq, nh + 1) * np.sqrt(2./(nh + 1))
    ...
    def construct_input(self):
        ...
        # Compute match value:
        Sproj = self.W_Sl.dot(self.l_sens).reshape( (self.nblocks, self.block_size) )
    ...
    def update_memory(self, zvec):
        ...
        if not zvec[-1] == 1:
            ...
            l = self.l_sens     #this excludes bias- and  match
