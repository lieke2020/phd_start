...
class WorkMATe(object):
    ...
    [q_int|q_ext]
    """
    def __init__(self, env=None, nhidden=15, nblocks=2, block_size=15, beta2 = 1, beta3 = 1):
        ...
        self.sigmoid_offset = 2.5
        self.transfer = lambda x: 1 / ( 1. + np.exp(self.sigmoid_offset - x) )
        self.dtransfer= lambda x: x * (1. - x) # derivative
        
        # tanh transfer function
        self.tanh_offset = 1
        self.transfer_t = lambda x:1 + np.tanh(x - self.tanh_offset)
        self.dtransfer_t = lambda x: 1-(x - 1)**2
        ...
        self.W_hx_start = np.copy(self.W_hx)        
        # connections S->h
        ...
        #ADDED BY LJC
        self.Tag_W_Sx, self.Trace_W_Sx = zeros_(self.W_Sx), zeros_(self.W_Sx)
    ...
    def construct_input(self):
        ...
        matches = 1- scipy.spatial.distance.cdist( 
            ...
            metric = WorkMATe.match2) .diagonal()
    ...
    def update_weights(self):
        ...
        wt_pairs = ( ( self.W_hx, self.Tag_W_hx ), 
                     ...
                     ( self.W_Sx, self.Tag_W_Sx) ) #added by LJC
    ...
    def update_tags(self):
        ...
        self.Tag_W_hx +=  np.expand_dims(self.feedback, 1) *  self.Trace_W_hx 
